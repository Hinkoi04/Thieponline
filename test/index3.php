...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  :root{
    --bg-dark: #0a192f;    /* Xanh đen thẳm */
    --bg-panel: #112240;
    --gold: #fbbf24;       /* Vàng rực rỡ */
    --gold-light: #fef3c7;
    --bg-light: #f8fafc;   
    --text-main: #334155;
    --text-dark: #0f172a;
  ...
  body{ background:var(--bg-dark); font-family:var(--serif); display:flex; justify-content:center; -webkit-font-smoothing:antialiased; overflow-x:hidden; }
  body.locked{overflow:hidden}

  .wrapper{ max-width:430px; width:100%; background:var(--bg-light); position:relative; overflow-x:hidden; min-height:100vh; margin: 0 auto; box-shadow: 0 0 40px rgba(0,0,0,0.8); }

  /* ================= UI CONTROLS ================= */
  #scroll-progress { position: fixed; top: 0; left: 50%; transform: translateX(-50%); width: min(430px, 100vw); height: 4px; background: rgba(251,191,36,0.2); z-index: 99999; }
  ...
  #music-btn { position: fixed; bottom: 30px; right: calc(50vw - 195px); z-index: 9999; width: 44px; height: 44px; background: rgba(10,25,47,0.9); border-radius: 50%; display: flex; justify-content: center; align-items: center; box-shadow: 0 4px 15px rgba(251,191,36,0.4); cursor: pointer; border: 1.5px solid var(--gold); animation: spinDisc 4s linear infinite; animation-play-state: paused; }
  @media (max-width: 430px) { #music-btn { right: 20px; } }
  #music-btn.playing { animation-play-state: running; }
  #music-btn::before { content: '🎵'; font-size: 1.2rem; filter: grayscale(100%) sepia(100%) hue-rotate(15deg) saturate(400%) brightness(1.1); }
  @keyframes spinDisc { 100% { transform: rotate(360deg); } }

  #toast { position: fixed; bottom: -100px; left: 50%; transform: translateX(-50%); background: rgba(10, 25, 47, 0.95); color: var(--gold-light); padding: 14px 28px; border-radius: 50px; font-family: var(--sans); font-size: 0.85rem; font-weight: 500; z-index: 999999; transition: bottom 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55); backdrop-filter: blur(10px); white-space: nowrap; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid var(--gold); }
  ...
  /* ========= HIỆU ỨNG MỞ "CÁNH CỬA TƯƠNG LAI" ========= */
  #intro-overlay{ position:fixed; top:0; left:50%; transform:translateX(-50%); width: min(430px, 100vw); height: 100vh; z-index: 99999; cursor:pointer; overflow:hidden; perspective: 1000px; }
  
  .gate { position: absolute; top: 0; width: 50%; height: 100%; background: linear-gradient(135deg, #112240 0%, #0a192f 100%); z-index: 2; transition: transform 1.6s cubic-bezier(0.77, 0, 0.175, 1); display: flex; align-items: center; }
  .gate-left { left: 0; border-right: 2px solid var(--gold); transform-origin: left center; justify-content: flex-end; }
  .gate-right { right: 0; border-left: 2px solid var(--gold); transform-origin: right center; justify-content: flex-start; }
  
  /* Họa tiết cánh cửa */
  .gate-decor { width: 80%; height: 90%; border: 1px solid rgba(251,191,36,0.3); position: relative; }
  .gate-left .gate-decor { margin-right: 15px; border-right: none; }
  .gate-right .gate-decor { margin-left: 15px; border-left: none; }

  /* Con dấu Học thuật ở giữa */
  .center-seal { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 120px; height: 120px; background: var(--bg-dark); border-radius: 50%; z-index: 4; border: 4px solid var(--gold); box-shadow: 0 0 30px rgba(251,191,36,0.5), inset 0 0 20px rgba(251,191,36,0.2); display: flex; flex-direction: column; justify-content: center; align-items: center; transition: all 1.2s cubic-bezier(0.2, 0.8, 0.2, 1); }
  .center-seal::before { content: ''; position: absolute; inset: 4px; border: 1px dashed var(--gold); border-radius: 50%; animation: spinSeal 10s linear infinite; }
  .seal-icon { font-size: 2.2rem; filter: drop-shadow(0 0 5px rgba(251,191,36,0.8)); margin-bottom: 2px; }
  .seal-text { font-family: var(--sans); font-size: 0.65rem; font-weight: 700; color: var(--gold-light); letter-spacing: 3px; }
  @keyframes spinSeal { 100% { transform: rotate(360deg); } }

  .click-hint{ position:absolute; bottom:60px; left:50%; transform:translateX(-50%); z-index:6; font-family:var(--sans); font-size:.65rem; color:var(--gold-light); text-transform:uppercase; letter-spacing:4px; animation:pulseHint 2s ease-in-out infinite; white-space:nowrap; text-shadow: 0 2px 10px rgba(0,0,0,0.8); transition: opacity 0.5s; }
  @keyframes pulseHint{0%,100%{opacity:.5}50%{opacity:1; text-shadow: 0 0 15px var(--gold);}}

  /* Animation khi bấm mở thiệp */
  #intro-overlay.open .gate-left { transform: translateX(-100%); }
  #intro-overlay.open .gate-right { transform: translateX(100%); }
  #intro-overlay.open .center-seal { transform: translate(-50%, -50%) scale(1.5) rotate(180deg); opacity: 0; filter: blur(10px); }
  #intro-overlay.open .click-hint { opacity: 0; }
  ...
  .hero-bg-layer { position: absolute; inset: -20px; background-image:url('uploads/grad_hero.jpg'); background-size:cover; background-position:center; z-index: 0; filter: brightness(0.6) contrast(1.1); transform: scale(1.1); transition: transform 3s ease-out; }
  #main-content.visible .hero-bg-layer { transform: scale(1); } /* Hiệu ứng zoom out nền khi mở cửa */
  
  .hero::before{ content:''; position:absolute; inset:0; background:linear-gradient(180deg, rgba(10,25,47,0.1) 0%, rgba(10,25,47,0.95) 100%); z-index:1; }
  ...
  .hero h1{ font-family:var(--serif); font-size:clamp(2.8rem, 10vw, 4.5rem); font-weight:700; line-height:1.1; color:#fff; text-shadow:0 4px 20px rgba(0,0,0,0.8); margin:10px 0; text-transform: uppercase; }
  .hero h2{ font-family:var(--script); font-size:clamp(2.5rem, 8vw, 3.5rem); color:var(--gold-light); font-weight: normal; margin-top: 5px; }

  .invite-box{ margin-top:40px; padding:25px 40px; background:rgba(10,25,47,0.6); border:1px solid rgba(251,191,36,0.4); border-radius:4px; backdrop-filter:blur(10px); display:inline-block; max-width:90%; opacity:0; box-shadow:0 10px 30px rgba(0,0,0,0.5); }
  ...
  .divider-line{flex:1; height:1px; background:linear-gradient(90deg, transparent, var(--bg-dark), transparent); opacity: 0.2;}
  .divider-icon{color:var(--bg-dark); font-size:20px; font-family: var(--serif); opacity: 0.5;}
  ...
  .title-lg{ font-family:var(--serif); font-size:clamp(2.5rem, 9vw, 3.5rem); color:var(--bg-dark); line-height:1.1; margin:5px 0 30px; font-weight:700; }

  .quote-box { position: relative; padding: 40px 20px; background: #fff; border-radius: 12px; box-shadow: 0 15px 40px rgba(10,25,47,0.06); border: 1px solid rgba(251,191,36,0.2); }
  ...
  /* BĂNG RÔN MỜI KHÁCH */
  .navy-band{ background: linear-gradient(135deg, var(--bg-panel) 0%, var(--bg-dark) 100%); padding:65px 28px; text-align:center; position:relative; overflow:hidden; border-top: 2px solid var(--gold); border-bottom: 2px solid var(--gold); box-shadow: 0 10px 30px rgba(0,0,0,0.2); }
  ...
  .date-block{ display:flex; justify-content:center; align-items:center; padding:35px 0; margin:25px 0; gap:30px; border-top:1px solid rgba(10,25,47,0.1); border-bottom:1px solid rgba(10,25,47,0.1); }
  ...
  .d-item strong{font-size:1.8rem; font-weight:300; color:var(--bg-dark)}
  .d-item.main strong{font-size:4.5rem; font-weight:700; color:var(--gold); line-height:1; font-family: var(--serif);}
  .date-sep{width:1px; height:60px; background:var(--gold); opacity: 0.4; transform: rotate(15deg);}

  .location-card{ background:#fff; border:1px solid rgba(251,191,36,0.3); padding:30px 25px; text-align:center; margin:35px 0 25px; box-shadow: 0 15px 35px rgba(10,25,47,0.05); position: relative; }
  .location-card::before { content:''; position:absolute; top:-5px; left:-5px; right:-5px; bottom:-5px; border: 1px solid rgba(251,191,36,0.1); pointer-events: none; }
  .loc-name{font-family:var(--serif); font-size:1.4rem; font-weight:700; color:var(--bg-dark); margin-bottom:12px; line-height: 1.3;}
  .loc-addr{font-family:var(--sans); font-size:.85rem; color:var(--text-main); line-height: 1.6; margin-bottom:25px}
  
  .btn{ display:inline-block; padding:16px 35px; background:var(--bg-dark); color:var(--gold-light); text-decoration:none; font-family:var(--sans); font-size:.75rem; font-weight: 600; text-transform:uppercase; letter-spacing:3px; transition:all .3s ease; border: 1px solid var(--bg-dark); }
  .btn:hover{background:var(--gold); color:var(--bg-dark); border-color: var(--gold); box-shadow: 0 10px 20px rgba(251,191,36,0.3); }

  /* TIMELINE */
  .timeline{ text-align:left; position: relative; padding: 20px 0 20px 20px; margin: 0 auto; width: 90%; border-left: 2px solid rgba(251,191,36,0.4); }
  .t-item{ position:relative; padding-bottom: 40px; padding-left: 25px; }
  .t-item:last-child { padding-bottom: 0; }
  .t-item::before { content:''; position:absolute; left:-33px; top:3px; width:16px; height:16px; background:var(--gold); border-radius:50%; box-shadow: 0 0 0 5px var(--bg-light), 0 0 15px rgba(251,191,36,0.5); }
  .t-time{font-family:var(--sans); font-weight:700; font-size:1rem; margin-bottom:8px; color:var(--bg-dark); letter-spacing: 1px;}
  .t-desc{font-family:var(--serif); font-size:1.2rem; color:var(--text-main); font-style: italic;}

  /* RSVP FORM */
  .rsvp-wrap{background:var(--bg-dark); padding:70px 28px; text-align: center; color: #fff; background-image: radial-gradient(circle at top right, rgba(251,191,36,0.1), transparent 50%); }
  .rsvp-wrap .title-lg { color: var(--gold-light); }
  .rsvp-wrap .text-desc { color: #cbd5e1; margin-bottom: 35px; }
  
  .form-control{ width:100%; padding:18px 20px; margin-bottom:18px; border:1px solid rgba(251,191,36,0.3); background:rgba(255,255,255,0.03); font-family:var(--sans); font-size:.9rem; color:#fff; outline:none; transition: all 0.3s; border-radius: 4px; }
  .form-control::placeholder { color: rgba(255,255,255,0.5); }
  .form-control:focus{ background:rgba(255,255,255,0.08); border-color: var(--gold); }
  textarea.form-control{resize:vertical; min-height:120px}
  .btn-submit{ width:100%; padding:20px; background:var(--gold); color:var(--bg-dark); border:none; font-family:var(--sans); font-size:.8rem; font-weight: 700; letter-spacing:4px; text-transform:uppercase; cursor:pointer; transition:all .3s; border-radius: 4px; }
  .btn-submit:hover{background:#fcd34d; transform:translateY(-3px); box-shadow:0 15px 25px rgba(0,0,0,0.3)}

  /* COUNTDOWN & FOOTER */
  .thank-section{ padding:80px 28px; text-align:center; background:var(--bg-light); border-top: 1px solid rgba(10,25,47,0.1); }
  ...
  .cd-num{ font-size:2.8rem; font-weight:700; color:var(--bg-dark); line-height:1; }
  ...
  .footer-name{ font-family:var(--serif); font-size:clamp(2rem,7vw,3rem); font-weight: 700; color:var(--bg-dark); line-height: 1.2; text-transform: uppercase;}
...
<!-- MÀN HÌNH CHỜ MỚI: CÁNH CỬA TƯƠNG LAI -->
<div id="intro-overlay">
  <!-- Cửa trái -->
  <div class="gate gate-left">
     <div class="gate-decor"></div>
  </div>
  <!-- Cửa phải -->
  ...
  <div class="center-seal">
     ...
     <div class="seal-text">STU</div>
  </div>
  
  <div class="click-hint">✦ Chạm để bước vào ✦</div>
</div>
...
<div class="wrapper" id="main-content">
  ...
  <div class="section reveal" style="padding-top:10px">
    <div class="quote-box">
      ...
      <p class="text-desc" style="font-size: 1.1rem; font-family: var(--serif); font-style: italic; color: var(--bg-dark); font-weight: 600;">
        Giáo dục là vũ khí mạnh nhất mà bạn có thể dùng để thay đổi thế giới.
      </p>
      <p class="text-desc" style="margin-top: 15px;">
        Hành trình nỗ lực với những dòng code tại giảng đường đại học đã chính thức khép lại. Ngày hôm nay không chỉ là đích đến, mà còn là khởi đầu cho những dự án lớn lao hơn trong sự nghiệp Lập trình Web. Sự hiện diện của bạn là niềm vinh hạnh và động lực to lớn đối với Hin trong cột mốc trưởng thành này.
      </p>
    </div>
  </div>
  ...
  <div class="cal-wrap">
    <div class="reveal">
      ...
      <div class="title-lg">Tháng Mười</div>
    </div>

    <div class="date-block reveal-scale">
      <div class="d-item"><span>Thứ Sáu</span><strong>16</strong></div>
      <div class="date-sep"></div>
      <div class="d-item main"><span>Tháng 10</span><strong>10</strong></div>
      <div class="date-sep"></div>
    ...
    <div class="location-card reveal">
      ...
      <div class="loc-name">Hội Trường STU</div>
  ...
  <div class="thank-section">
    ...
    <div class="divider reveal"><div class="divider-line"></div><div class="divider-icon">♦</div><div class="divider-line"></div></div>
    <div class="title-sm reveal" style="margin-top: 40px; color: var(--bg-dark);">Trân Trọng Cảm Ơn</div>
...
(function(){
  ...
  /* ---- COUNTDOWN DATE ---- */
  const target = new Date('2026-10-16T07:30:00').getTime(); 
  
  function updateCD(){
    const now = Date.now(), d = target - now;
    if(d < 0){
      document.getElementById('countdown').innerHTML='<div style="font-family:var(--serif); font-size:1.5rem; color:var(--bg-dark); font-weight:700; letter-spacing:2px">Sự Kiện Đã Diễn Ra</div>';
      return;
    }
  ...
  const colors = ['#fbbf24', '#fef3c7', '#0a192f', '#ffffff'];
  ...
  /* ---- MỞ THIỆP & KÍCH HOẠT HIỆU ỨNG CÁNH CỬA TƯƠNG LAI ---- */
  ...
  if(intro) {
      intro.addEventListener('click', function(){
        ...
        // Kích hoạt class open để CSS transition xử lý cánh cửa
        intro.classList.add('open');

        // Hiển thị nội dung phía sau từ từ
        setTimeout(()=>{
          main.classList.add('visible');
        }, 300);

        // Sau khi cửa mở xong (1.6s), ẩn overlay đi để tiết kiệm tài nguyên
        setTimeout(()=>{
          ...
          // Trễ thêm 1 giây ngắm Hero rồi mới tự cuộn
    ...
    if(!ticking) {
      window.requestAnimationFrame(() => {
        ...
        if(scrollTop < window.innerHeight) {
          if(heroBg) heroBg.style.transform = `scale(1) translateY(${scrollTop * 0.4}px)`;
